  ...
  for (let word of words) {
    ...
    for (let char of word) {
      ...
      let targetSw = strokeWeightVal;
        ...
        // Color also interpolates based on proximity
        targetCol = [
            lerp(color[0], interactiveColor[0], proximity),
            lerp(color[1], interactiveColor[1], proximity),
            lerp(color[2], interactiveColor[2], proximity)
        ];
        }
...
function drawLetter(idArray, p, widthh) {
  let a = new Quadrilateral(widthh, 10); // Use consistent 'n' if needed elsewhere
